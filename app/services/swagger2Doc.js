...
function addRelationSwagger2Doc(association, controllers) {
    ...
    if (association.isSourceMultiple() && association.isTargetMultiple()) { // [Many to Many]
        buildManyRoleDocs(sourceController, association.bClass, association.aRole, false, true);
        buildManyRoleDocs(targetController, association.aClass, association.bRole, false, true);
    }
    else if (!association.isSourceMultiple() && !association.isTargetMultiple()) { // [One to One]
        if (association.isSourceOptional()) {
            buildManyRoleDocs(sourceController, association.bClass, association.aRole, true, association.isTargetOptional());
            ...
            buildManyRoleDocs(targetController, association.aClass, association.bRole, true, association.isSourceOptional());
        }
    }
    else { // [One to Many], [Many to One]
        if (association.isSourceMultiple()) {
            buildManyRoleDocs(sourceController, association.bClass, association.aRole, false, association.isTargetOptional());
        }
        else {
            buildOneCardinalityApis(sourceController, association.aClass, association.bClass, association.aRole, association.bRole);
        }
        if (association.isTargetMultiple()) {
            buildManyRoleDocs(targetController, association.aClass, association.bRole, false, association.isSourceOptional());
        }
        else {
            buildOneCardinalityApis(targetController, association.bClass, association.aClass, association.bRole, association.aRole);
        }
...
function buildManyRoleDocs(controller, targetName, sourceRole, singleResult, optional) {
